...
public interface PatientDao {
    ...
    @Query("UPDATE patient_info SET breakfast_items = :items, breakfast_juices = :juices, breakfast_drinks = :drinks WHERE patient_id = :patientId")
    int updateBreakfastItems(long patientId, String items, String juices, String drinks);

    @Query("UPDATE patient_info SET lunch_items = :items, lunch_juices = :juices, lunch_drinks = :drinks WHERE patient_id = :patientId")
    int updateLunchItems(long patientId, String items, String juices, String drinks);

    @Query("UPDATE patient_info SET dinner_items = :items, dinner_juices = :juices, dinner_drinks = :drinks WHERE patient_id = :patientId")
    ...
    @Query("SELECT * FROM patient_info WHERE room_number = :room AND wing = :wing LIMIT 1")
    PatientEntity getPatientByRoomAndWing(String room, String wing);
    ...
    @Query("SELECT COUNT(*) FROM patient_info WHERE wing = :wing AND room_number = :room")
    int countByWingAndRoom(String wing, String room);

    @Query("SELECT DISTINCT wing FROM patient_info ORDER BY wing")
    LiveData<List<String>> getAllWingsLive();

    @Query("SELECT DISTINCT wing FROM patient_info ORDER BY wing")
    List<String> getAllWings();
}